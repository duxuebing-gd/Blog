---
title: 'PETS3 Day 2：U-1.2 主系表 + 词汇 #16–#35'
description: '2026-06-20 学习记录。语法 U-1.2 主系表；乱序词汇第 16–35 号；15 例句 + 语法摘要 + 博客笔记。'
updatedDate: 2026-06-20
pubDate: 2026-06-20
category: english
tags: [pets3, day-2, u-01, grammar, linking-verb, vocabulary]
---

## 今日对照

| 项目 | 内容 | 完成 |
|------|------|------|
| Day | **D2** / 85 | ☑ |
| 日期 | 2026-06-20（周六） | |
| 时长 | **2 h** | |
| 复习 | [D1 错题本](/blog/english/pets3/u-01-subject-verb-object-day1/#错题本--d2-复习清单) | ☐ |
| 语法 | 语法 PDF → **U-1.2 主系表** | ☑ |
| 词汇 | 乱序 PDF → **#16–#35**（20 词） | ☐ |
| 产出 | 句型判断 + 选词填空 + 语法查错 | ☑ |

---

## 一句话讲清楚

> **主系表 = 主语 + 系动词 + 表语**：系动词不表示具体动作，表语说明主语「是什么 / 怎么样」。  
> 例：**She is happy.**（她很开心）— 不是「她做了 happy 这个动作」。

---

## 和 Day 1 主谓宾对比

| 句型 | 结构 | 中间动词 | 后面是什么 | 例句 |
|------|------|----------|------------|------|
| **主谓宾** | S + V + O | **实义动词**（有动作） | **宾语**（动作对象） | I **drink** milk. |
| **主系表** | S + V + P | **系动词**（无实义动作） | **表语**（描述主语） | She **is** happy. |

**D1 错题 #2 就是这里**：She is happy → **主系表**，不是主谓。

---

## 语法 · U-1.2 主系表

### 什么是系动词？

常见系动词（PDF U-1.2）：

| 类型 | 动词 | 例句 |
|------|------|------|
| be 动词 | am / is / are / was / were | He **is** a student. |
| 感官 | look, sound, smell, taste, feel | The soup **smells** good. |
| 变化 | become, get, turn, grow | The leaves **turn** yellow. |
| 持续 | keep, remain, stay | Please **keep** quiet. |

### 什么可以当表语？

- **形容词**：She is **happy**. / He looks **tired**.
- **名词**：He is **a teacher**. / This is **my book**.
- **介词短语**：The book is **on the desk**.

### 易错点（PDF 提醒）

1. **系动词 ≠ 实义动词** — `is` 不是「做」某个动作  
2. **be 和主语要一致** — I am / He is / They are  
3. **表语词性要对** — 系动词后常接 adj. 或 n.，不接副词（❌ She is happily → ✅ She is happy）

### 我造的语法例句（至少 3 句，尽量用今天词汇）

| # | 句型 | 我的句子 |
|---|------|----------|
| 1 | be + adj. | <!-- She is sensible. --> |
| 2 | be + n. | <!-- He is a typist. --> |
| 3 | 感官系动词 + adj. | <!-- The evening air feels cool. --> |
| 4 | （可选）和主谓宾对比 | <!-- I attend class.（主谓宾）vs The class is central.（主系表）--> |

### 课后题 · 句型判断（U-1.2 第三节）— **6/10**

| # | 句子 | 我的答案 | 正确答案 | |
|---|------|----------|----------|---|
| 1 | He is a student. | 主系表 | 主系表 | ✅ |
| 2 | She eats apples. | 主谓宾 | 主谓宾 | ✅ |
| 3 | The cake tastes good. | 主谓宾 | **主系表** | ❌ |
| 4 | We buy books for children. | 主谓宾 | 主谓宾 | ✅ |
| 5 | They are busy. | 主系表 | 主系表 | ✅ |
| 6 | This song sounds nice. | 主谓宾宾补 | **主系表** | ❌ |
| 7 | She is in the park. | 主系表 | 主系表 | ✅ |
| 8 | I love reading. | 主谓表 | **主谓宾** | ❌ |
| 9 | My father tells me a story. | 主谓宾宾补 | **主谓宾宾** | ❌ |
| 10 | The game makes us excited. | 主谓宾宾补 | 主谓宾宾补 | ✅ |

**今日错因**：

| 错因 | 涉及题号 | 记住 |
|------|----------|------|
| **感官系动词当动作动词** | #3 #6 | taste / sound / smell / look / feel + **adj.** → **主系表** |
| **love 是及物动词** | #8 | I love reading. → reading 是**宾语**（动名词），不是表语 |
| **tell 双宾结构** | #9 | tell **me** **a story** = 给谁 + 什么 → **主谓宾宾** |

**感官系动词速记**（#3、#6 错在这里）：

```
The cake tastes good.   = 蛋糕尝起来好  → 主系表（不是「蛋糕吃 good」）
This song sounds nice.  = 歌听起来好听  → 主系表（不是宾补）
```

### 语法摘要 3 句（中文）

1. **系动词**（is / look / sound / taste / feel…）后面接形容词或名词，说明主语「是什么 / 怎么样」→ **主系表**。
2. **感官动词 + 形容词**（tastes good, sounds nice）是主系表，不是主谓宾。
3. **tell / give sb sth** = 主谓宾宾；**make sb adj** = 主谓宾宾补 — #9 和 #10 的区别。

### 课后题 · 选词填空（U-1.2 第二节）— **3/8**

备选词（各用一次）：is · are · look · eat · run · enjoy · give · make

| # | 句子 | 我的答案 | 正确答案 | |
|---|------|----------|----------|---|
| 1 | The flowers ___ beautiful. | look | look | ✅ |
| 2 | He ___ a teacher. | is | is | ✅ |
| 3 | We ___ noodles for breakfast. | enjoy | **eat** | ❌ |
| 4 | She ___ every afternoon. | eat | **runs** | ❌ |
| 5 | They ___ my good friends. | are | are | ✅ |
| 6 | I ___ playing basketball with friends. | make | **enjoy** | ❌ |
| 7 | My teacher ___ me a pen and a book. | give | **gives** | ❌ 词对，形式错 |
| 8 | The news ___ us happy. | look | **makes** | ❌ |

**参考答案**：

```
1. The flowers look beautiful.
2. He is a teacher.
3. We eat noodles for breakfast.
4. She runs every afternoon.
5. They are my good friends.
6. I enjoy playing basketball with friends.
7. My teacher gives me a pen and a book.
8. The news makes us happy.
```

**错因**：look 用了两次；run/eat 未用；#8 应是 **make us happy**（宾补），不是 look。

### 课后题 · 语法查错（U-1.2 第三节）

**做题三步**：找主语 → 单数还是复数 → 有没有 yesterday 等时间词。

| # | 原句 | 改正 | 错因 |
|---|------|------|------|
| 1 | Lily **are** a kind and happy girl. | Lily **is**… | be 动词：Lily 单数 → is ✅ 已改对 |
| 2 | She **like** drawing… | She **likes**… | 三单 ✅ 已改对 |
| 3 | Her mother **give**… **yesterday**. | Her mother **gave**… | **yesterday → 过去式 gave**（不是 gives） |
| 4 | Lily **draw** a small cat… | Lily **draws**… | She/Lily 三单 |
| 5 | She **show** the picture… | She **shows**… | She 三单 |
| 6 | All her **classmate thinks**… | All her **classmates think**… | 名词复数 + 复数动词 |
| 7 | Her teacher **let** her keep… | Her teacher **lets** her keep… | teacher 三单 |
| 8 | Lily **feel** very proud… | Lily **feels**… | Lily 三单 |
| 9 | She **say** thanks… | She **says**… | She 三单 |
| 10 | Her parents **loves** her… | Her parents **love** her… | parents 复数，动词不加 s |

**口诀**：

```
Lily / She / teacher  → likes, draws, feels, lets, says
parents / classmates  → love, think（不加 s）
yesterday             → gave（过去式）
Lily + be             → is（不是 are）
```

---

## 词汇 · 乱序 #16–#35

> 尽量用 **主系表** 句型造句（系动词 + 形容词/名词）

| # | 单词 | 音标 | 释义 | 我的例句 |
|---|------|------|------|----------|
| 16 | approximate | /əˈprɒksɪmət/ | adj. 大约的 | |
| 17 | attend | /əˈtend/ | v. 出席 | |
| 18 | radiate | /ˈreɪdieɪt/ | v. 辐射 | |
| 19 | predict | /prɪˈdɪkt/ | v. 预测 | |
| 20 | harbour | /ˈhɑːbə/ | n. 港口 | |
| 21 | box | /bɒks/ | n. 盒子 | |
| 22 | type | /taɪp/ | n. 类型 | |
| 23 | bucket | /ˈbʌkɪt/ | n. 水桶 | |
| 24 | evening | /ˈiːvnɪŋ/ | n. 晚上 | |
| 25 | adjective | /ˈædʒɪktɪv/ | n. 形容词 | |
| 26 | sweat | /swet/ | n. 汗 | |
| 27 | exactly | /ɪɡˈzæktli/ | adv. 确切地 | |
| 28 | daily | /ˈdeɪli/ | adj. 每日的 | |
| 29 | trim | /trɪm/ | adj. 整齐的 | |
| 30 | nevertheless | /ˌnevəðəˈles/ | adv. 然而 | |
| 31 | storage | /ˈstɔːrɪdʒ/ | n. 存储 | |
| 32 | typist | /ˈtaɪpɪst/ | n. 打字员 | |
| 33 | central | /ˈsentrəl/ | adj. 中心的 | |
| 34 | entrance | /ˈentrəns/ | n. 入口 | |
| 35 | sensible | /ˈsensəbl/ | adj. 明智的 | |

**主系表造句提示**（#28 daily、#29 trim、#33 central、#35 sensible 都是形容词，很适合）：

- The plan is **sensible**.
- This room is **central**.
- His desk is **trim** and tidy.

### 今日最难的 5 个词

1. 
2. 
3. 
4. 
5. 

---

## D1 错题速复习（10 min，开课前）

- [ ] She is happy → **主系表**（今天重点）
- [ ] I tell / he tells — 三单口诀
- [ ] give sb sth = 主谓宾宾；make sb adj = 主谓宾宾补

---

## 费曼检验

- [x] 主系表 be 动词句（句型判断 #1 #5 #7）判断正确
- [ ] **taste / sound + adj.** 仍易错 → D3 先复习
- [ ] tell sb sth vs make sb adj → D3 复习
- [ ] **yesterday → 过去式**（语法查错 #3）→ D3 复习
- [ ] 复数主语动词不加 s（#6 #10）→ D3 复习

**D3 开课前 10 min 重做**：

- 句型判断 #3 #6 #8 #9（闭卷）
- 语法查错 #3 #6 #10（闭卷）
- 选词填空 #4 #6 #8（闭卷）

---

## D2 错题本 · D3 复习清单

| 优先级 | 知识点 | 来自 |
|--------|--------|------|
| ★★★ | yesterday → **gave** 不是 gives | 语法查错 #3 |
| ★★★ | 感官系动词 + adj. = 主系表 | 句型 #3 #6 |
| ★★ | parents/classmates 复数 → 动词原形 | 语法查错 #6 #10 |
| ★★ | make sb adj ≠ look sb adj | 选词填空 #8 |
| ★★ | tell/give sb sth = 主谓宾宾 | 句型 #9 |
| ★ | enjoy doing vs make sb do | 选词填空 #6 |

---

## 学习日志

| 开始时间 | 结束时间 | 实际时长 | 备注 |
|----------|----------|----------|------|
| — | — | ~2 h | 句型 6/10 · 选词 3/8 · 语法查错已学 |

*上一篇：[Day 1 · U-1.1 主谓宾](/blog/english/pets3/u-01-subject-verb-object-day1/) · 计划：[85 天每日清单](/blog/english/pets3-2026-study-plan/) · 明天 D3：U-1.2 课后题*
